...
import 'package:omsetin_resto/model/cashier.dart';
import 'package:omsetin_resto/model/category.dart';
import 'package:omsetin_resto/model/expenceModel.dart';
...
import 'package:omsetin_resto/model/setting.dart';
import 'package:omsetin_resto/model/stock_addition.dart';
import 'package:omsetin_resto/model/transaction.dart';
...
class DatabaseService {
  ...
  int pembulatanHargaJual(int hargaJual) {
    int sisa = hargaJual % 1000;

    if (sisa == 0) return hargaJual;

    if (sisa <= 500) {
      return hargaJual - sisa + 500;
    } else {
      return hargaJual - sisa + 1000;
    }
  }
  ...
  Future<void> updateAllProductPricesFromSetting() async {
    ...
    for (final product in products) {
      ...
      final hargaJualMentah = (hargaBeli * (1 + percentage / 100)).round();
      final hargaJual = pembulatanHargaJual(hargaJualMentah);
  ...
  Future<Map<String, int>> getTotalStock() async {
    ...
    return {'totalStock': totalstock, 'totalNilaiStock': totalnilai};
  }
  ...
  Future<List<String>> getDistinctProductCategories() async {
    final db = await database;
    final List<Map<String, dynamic>> maps = await db.rawQuery(
        'SELECT DISTINCT category_name FROM products WHERE category_name IS NOT NULL');
    return maps.map((map) => map['category_name'] as String).toList();
  }
  ...
  Future<void> addProductStock(Map<String, dynamic> stockAdditionData) async {
    final db = await database;
    await db.insert(_stockAdditionTable, {
  ...
  Future<void> deleteExpense(int expenseId) async {
    ...
    try {
      ...
      print("Berhasil menghapus expense dengan id $expenseId");
    } catch (e) {
      print("Error $e");
    }
  }
  ...
  Future<String?> getsettingProfitType() async {
    final db = await database;
    final result = await db.query(_settingTable, columns: [_settingProfitType]);
    if (result.isNotEmpty) {
  ...
  Future<void> updatesettingProfitType(String newProfit) async {
    final db = await database;
    await db.update(_settingTable, {_settingProfitType: newProfit});
  }

  Future<double> getSettingProfitOrZero() async {
    final db = await database;
    final result = await db.query(_settingTable, columns: [_settingProfit]);
  ...
  Future<void> updateSettingProfit(double value) async {
    final db = await database;
    await db.update(_settingTable, {_settingProfit: value.toString()});
  }
  ...
  Future<Map<String, String?>> getSettingReceipt() async {
    ...
    if (result.isNotEmpty) {
      debugPrint("Columns available: ${result.first.keys}");
      return {
  ...
  Future<void> updateSettingTemplate(String newTemplate, String newSize) async {
    final db = await database;
    print("Updating receipt template to: $newTemplate, size: $newSize");

    try {
      await db.update(_settingTable, {
        _settingReceipt: newTemplate,
        _settingReceiptSize: newSize,
      });
      print("Success, updated receipt template and size");
    } catch (e) {
      print("Error updating receipt template: $e");
    }
  }
  ...
  Future<void> deleteIncome(int incomeId) async {
    ...
    try {
      ...
      print("Berhasil menghapus income dengan id $incomeId");
    } catch (e) {
      print("Error $e");
    }
  }
  ...
  Future<Map<String, dynamic>?> getCurrentCashier() async {
    final db = await database;
    final data = await db.query(_currentCashierTable);
