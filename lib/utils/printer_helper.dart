  ...
  static Future<void> printReceiptAndOpenDrawer(
    ...
    try {
      ...
      if (transactionId != null) {
        transaction = await _databaseService.getTransactionById(transactionId);
        if (transaction != null) {
          lastTransaction = transaction;
        } else {
          final allTransactions = await _databaseService.getTransaction();
          lastTransaction = allTransactions.last;
        }
      } else {
        final allTransactions = await _databaseService.getTransaction();
        lastTransaction = allTransactions.last;
    ...
    try {
      ...
      if (codeType == 'barcode') {
        ...
        bytes += generator.text(formattedPrice ?? '',
            styles: PosStyles(align: PosAlign.center));
        bytes += generator.feed(6);
      } else if (codeType == 'qrcodeNamaHarga') {
        bytes += generator.feed(6);
        bytes += generator.qrcode(codeText ?? '', size: QRSize.size6);
        bytes += generator.feed(2);
        bytes += generator.text(productName ?? '',
