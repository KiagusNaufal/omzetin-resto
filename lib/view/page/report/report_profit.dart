...
class _ReportProfitState extends State<ReportProfit> {
  ...
  Future<void> prepareExcelData() async {
    try {
      ...
      final profitKotor = omzet - totalModal;
      ...
      print("❌ Error preparing Excel data: $e");
    }
  }

// Fungsi untuk UI data (format list terpisah)
  Future<List<Map<String, dynamic>>> _loadReportData() async {
    await prepareExcelData(); // Siapkan data Excel terlebih dahulu

    if (reportExcel.isEmpty) return [];

    final data = reportExcel.first;
    return [
      {
        'teksLaporan': 'Omzet',
        'nominal': currencyFormatter.format(data['omzet'])
      },
      {
        'teksLaporan': 'Modal Produk',
        'nominal': currencyFormatter.format(data['totalModal'])
      },
      {
        'teksLaporan': 'Total Pengeluaran',
        'nominal': currencyFormatter.format(data['totalExpense'])
      },
      {
        'teksLaporan': 'Profit Kotor',
        'nominal': currencyFormatter.format(data['profitKotor'])
      },
      {
        'teksLaporan': 'Profit Bersih',
        'nominal': currencyFormatter.format(data['profitBersih'])
      },
    ];
  }
